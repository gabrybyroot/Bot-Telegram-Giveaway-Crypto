...
<b>!!! IMPORTANT !!!</b>
<i>
By using the command <code>/accept</code>, you assume responsibility for every action taken. (project name) is not responsible for errors during registration or possible fraud occurring between users during the Giveaway.
...
htmlText2 = '''
<b>Prize for the first-ranked worth $300</b>

<i>The prize will be directly deposited into the Wallet Address declared during the Giveaway.</i>

<u>REMEMBER!!!</u>

<i>The prize is just a gift from (project name), not an investment or payment for a service. We remind you that the Giveaway is public, (project name) will publish the transaction of the gift sent to the first-ranked and their name in the global list for transparency.</i>
'''
...
async def Walletbep20(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await updateFile()
    uuid = update.message.from_user['id']
    if sheet[f"C{userRow(uuid)}"].value == None:
        await ctx.bot.send_message(chat_id=update.effective_chat.id, text="Setting your Wallet Address Bep-20: " + ''.join(ctx.args))
        pushData(uuid, "C", ''.join(ctx.args))
    else:
        await ctx.bot.send_message(chat_id=update.effective_chat.id, text="I updated your Wallet Address Bep-20: " + ''.join(ctx.args))
        pushData(uuid, "C", ''.join(ctx.args))
...
async def getData(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uuid = update.message.from_user['id']
    row = userRow(uuid)
    tgName, Walletbep20, acceptedConditions, getlink= sheet[f"B{row}"].value, sheet[f"C{row}"].value, sheet[f"D{row}"].value, sheet[f"E{row}"].value
    baseString = f"Your data summary:\n<b>Telegram Username:</b> {tgName}\n<b>Address Bep-20:</b> {Walletbep20}\n<b>Accept Terms and Conditions:</b> {'&#10060;' if not bool(acceptedConditions) else '&#9989;'}\n<b>Referral link:</b> {getlink}"
...
if __name__ == "__main__":
    ...
    app.add_handlers(
        [
            ...
            CommandHandler("wallet", Walletbep20),
